...
$bool = Main_Or_Detail_Page('houses');
...
if($bool)
{
	...
	$filt_var_res = All_Variants(17,'ADVANTAGES');
}
...
<div class="filter wrap">
	<div class="first-line">
		<h2>Дома в Новосибирске и области</h2>
		<?php 
		if($_GET['house_type'] != '')
		{
	     $hreff = '/map/?type=houses&house_type='.$_GET['house_type'].'&price-from='.$_GET['price-from'].'&price-to='.$_GET['price-to'];
		}
		else if($_GET['popular_ads'] != '')
		{
			$hreff = '/map/?type=houses&popular_ads='.$_GET['popular_ads'].'&price-from='.$_GET['price-from'].'&price-to='.$_GET['price-to'];
		}
		else
		{
		 $hreff = '/map/?type=houses&price-from='.$_GET['price-from'].'&price-to='.$_GET['price-to']; 
		}
		?>
	...
	<div class="sort">
		<span>Сортировать: </span>
            <div class="first-sel">
            	<?php if(!isset($_SESSION['type_of_sort_houses'])): ?>
					<span class="this-variant">Сначала дешевле</span>
				<?php elseif($_SESSION['type_of_sort_houses'] == 'ASC'): ?>
					<span class="this-variant">Сначала дешевле</span>
				<?php elseif($_SESSION['type_of_sort_houses'] == 'DESC'): ?>
...
<?php 
	...
	if(isset($_SESSION['type_of_sort_houses']))
	{
		$sortOrder = $_SESSION['type_of_sort_houses'];
	}
	else
	{
		$sortOrder = 'ASC';
...
<?php 
	$all_text = Return_All(
	Array("IBLOCK_ID"=>25, "ACTIVE"=>"Y", "PROPERTY_FOR_HOUSES_VALUE"=>"YES"),
	Array("ID", "IBLOCK_ID", "NAME", "PROPERTY_FOR_HOUSES", "PREVIEW_TEXT", "PREVIEW_PICTURE")
	);
?>
