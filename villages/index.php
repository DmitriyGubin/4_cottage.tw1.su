...
if($bool)
{
	...
	$houses_min_price = Return_Village_Min_Price(17);
	$PROPERTY_CODE = "MIN_HOUSE_PRICE";
	$IBLOCK_ID = 16;
	/********заполнение свойства минимальная цена дома************/
	foreach ($houses_min_price as $key => $house_price) 
	{
		CIBlockElement::SetPropertyValues($key, $IBLOCK_ID, $house_price, $PROPERTY_CODE);
	}
	/********заполнение свойства минимальная цена дома************/

	$areas_min_price = Return_Village_Min_Price(18);
	foreach ($areas_min_price as $key => $area) 
	{
		$house = $houses_min_price[$key];
		if(($area != '') && ($house != ''))
		{
			$min_price_something = min($area, $house);
		}
		if(($area == '') && ($house == ''))
		{
			$min_price_something = '';
		...
		if(($area != '') && ($house == ''))
		{
			$min_price_something = $area;
		}
		if(($area == '') && ($house != ''))
		{
			$min_price_something = $house;
		}

		CIBlockElement::SetPropertyValues($key, $IBLOCK_ID, $min_price_something, 'PRICE');
	}

	$filt_var_res = All_Variants(16,'ADVANTAGES');
}
...
<div class="filter wrap">
	<div class="first-line">
		<h2>Коттеджные поселки в Новосибирске и области</h2>
		<?php 
		if($_GET['village_type'] != '')
		{
			$hreff = '/map/?type=villages&village_type='.$_GET['village_type'].'&price-from='.$_GET['price-from'].'&price-to='.$_GET['price-to'];
		}
		else if($_GET['popular_ads'] != '')
		{
			$hreff = '/map/?type=villages&popular_ads='.$_GET['popular_ads'].'&price-from='.$_GET['price-from'].'&price-to='.$_GET['price-to'];
		}
		else
		{
			$hreff = '/map/?type=villages&price-from='.$_GET['price-from'].'&price-to='.$_GET['price-to'];
		}
		?>
	...
	<div class="sort">
		<span>Сортировать: </span>
            <div class="first-sel">
            	<?php if(!isset($_SESSION['type_of_sort_villa'])): ?>
					<span class="this-variant">Сначала дешевле</span>
				<?php elseif($_SESSION['type_of_sort_villa'] == 'ASC'): ?>
					<span class="this-variant">Сначала дешевле</span>
				<?php elseif($_SESSION['type_of_sort_villa'] == 'DESC'): ?>
            	...
            	<div class="var-box-first" style="display: none;">
            		<div id="ASC">Сначала дешевле</div>
            		<div id="DESC">Сначала дороже</div>
            	</div>
            </div>
        </div> 
</div>
...
<?php 
	$sortField = "property_MIN_HOUSE_PRICE";

	if(isset($_SESSION['type_of_sort_villa']))
	{
		$sortOrder = $_SESSION['type_of_sort_villa'];
	}
	else
	{
		$sortOrder = 'ASC';
 ...
 ?>
	...
	"villages-list", 
	array(
		"COMPONENT_TEMPLATE" => "villages-list",
		"IBLOCK_TYPE" => "catalog",
		"IBLOCK_ID" => "16",
		"NEWS_COUNT" => "3",
		"USE_SEARCH" => "N",
...
<?php 
	$all_text = Return_All(
	Array("IBLOCK_ID"=>25, "ACTIVE"=>"Y", "PROPERTY_FOR_VILLAGE_VALUE"=>"YES"),
	Array("ID", "IBLOCK_ID", "NAME", "PROPERTY_FOR_VILLAGE", "PREVIEW_TEXT", "PREVIEW_PICTURE")
	);
?>
    <div class="map wrap">
